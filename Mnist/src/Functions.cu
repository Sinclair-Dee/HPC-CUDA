...
//void ConvLayerForward(int bacth, host_vector<float>& FM_in,  int CH_in, int H_in, int W_in,
...
void GEMM(float *MatrixM, float* MatrixN, float* MatrixOut,
  ...
  for(int m = 0; m < M_width_N_height_in; m++){
    if(row < M_height_in && (m * TILE_WIDTH + threadidx) < M_width_N_height_in)
      shmemM[threadidy][threadidx] = MatrixN[row * M_width_N_height_in + (m*TILE_WIDTH) + threadIdx];
    else
      shmemM[threadidy][threadidx] = 0;
    if((m*TILE_WIDTH + threadidy) < M_width_N_height_in && col < N_width_in)
      shmemN[threadidy][threadidx] = MatrixM[(m*TILE_WIDTH + threadidy) * M_width_N_height_in +col];
    else
  ...
  if(row < height_out && col < width_out){
      MatrixOut[row * width_out + col] = Pvalue;
  }
}
__global__
void GEMMwithBias(float *MatrixM, float* MatrixN, float* MatrixOut, float* B,
  ...
  if(row < height_out && col < width_out){
      MatrixOut[row * width_out + col] = Pvalue + B[col];
  }
  }
__global__
void ConvLayerForwardGPUnaive(float *FM_in, float  *W, float *FM_out,
  ...
  //3D Blocks grid
  //Grid.X : minibatch(1000) samples in one batch
  //Grid.Y : CH_out output channel of feature maps
  ...
  //int W_TileNum = W_out / TILE_WIDTH;
  // number of vertical tiles per output map
  //int H_TileNum = H_out / TILE_WIDTH;
  int H_out = H_in - W_h_w +1;
  int TileNum = ceilf((float)W_out/TILE_WIDTH);
  TileNum = TileNum == 0 ? 1 : TileNum;
  int batch, ch_out, h, w, ch_in,i,j;
  batch  = blockIdx.x;
  ch_out = blockIdx.y;
  h = (blockIdx.z / TileNum) * TILE_WIDTH + threadIdx.y; //h of H_out
  w = (blockIdx.z % TileNum) * TILE_WIDTH + threadIdx.x; //w of W_out
...
void ConvLayerForwardGPUtiled(float *FM_in, float  *W, float *FM_out,
  ...
  //number of vertical tiles per output map
  //int H_TileNum = H_out / TILE_WIDTH;
  //noting that boundary check is needed
